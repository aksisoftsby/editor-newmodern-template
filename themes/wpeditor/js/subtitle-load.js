(function ($) {
  ...
  $(document).on("keydown", "#title, #the_subtitle", function (e) {
    ...
    if (9 == keyCode) {
      e.preventDefault();
      var target =
        $(this).attr("id") == "title" ? "#the_subtitle" : "textarea#content";
      if (
        target === "#the_subtitle" ||
        $("#wp-content-wrap").hasClass("html-active")
      ) {
        $(target).focus();
      } else {
