...
if (!function_exists('wpberita_setup')) :
	...
	function wpberita_setup()
	{
		 ...
		 */
		load_theme_textdomain('wpeditor', get_template_directory() . '/languages');
		...
		register_nav_menus(
			array(
				...
				'menu-6' => esc_html__('Menu sebelah burger', 'wpberita'),
			)
		);
...
?>
		<style<?php echo $type_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
...
function wpberita_scripts()
{
	...
	if (!empty($font_url)) {
		wp_enqueue_style('wpeditor-fonts', esc_url_raw($font_url), array(), WPBERITA_VERSION);
	}
	wp_enqueue_style('wpeditor-style', get_stylesheet_uri(), array(), filemtime(__DIR__ . "/style.css"));
	wp_enqueue_style('wpeditor-style-blb', get_template_directory_uri() . "/css/blbedit.css", array(), filemtime(__DIR__ . "/css/blbedit.css"));
	# wp_enqueue_style('wpeditor-style', get_stylesheet_uri(), array(), WPBERITA_VERSION);
	# wp_enqueue_style('wpeditor-style-blb', get_template_directory_uri() . "/css/blbedit.css", array(), time());
	# wp_enqueue_style('wpberita-blb-style', get_template_directory_uri() . "/css/blb.css", array(), WPBERITA_VERSION);
	# wp_enqueue_style('wpberita-blbnavi-style', get_template_directory_uri() . "/css/blbnavi.css", array(), WPBERITA_VERSION);
	# wp_enqueue_style('wpberita-blb-adaptive-style', get_template_directory_uri() . "/css/blb-adaptive.css", array(), WPBERITA_VERSION);

	if (wpberita_is_amp()) {
		wp_enqueue_style('wpeditor-ampstyle', get_template_directory_uri() . '/style-amp.css', array('wpberita-style'), WPBERITA_VERSION);
	}
	...
	if (!wpberita_is_amp()) {
		...
		if ($is_IE) {
			wp_enqueue_script('wpeditor-ponyfill', 'https://cdn.jsdelivr.net/npm/css-vars-ponyfill@2', array(), WPBERITA_VERSION, false);
		...
		wp_enqueue_script('wpeditor-main', get_template_directory_uri() . '/js/main.js', array(), WPBERITA_VERSION, true);
		$loadmore = get_theme_mod('gmr_blog_pagination', 'gmr-more');
		if (('gmr-infinite' === $loadmore) || ('gmr-more' === $loadmore)) {
			wp_enqueue_script('wpeditor-infscroll', get_template_directory_uri() . '/js/infinite-scroll-custom.js', array(), WPBERITA_VERSION, true);
			wp_localize_script(
				'wpeditor-infscroll',
				'gmrobjinf',
				array(
		...
		if (!$is_IE) {
			...
			if (0 === $darkmodebuttondisable) {
				wp_enqueue_script('wpeditor-darkmode', get_template_directory_uri() . '/js/darkmode.js', array(), WPBERITA_VERSION, true);
			}
		}
		...
		# if (0 === $modulehome) {
		wp_enqueue_script(
			'wpeditor-tinyslider-module',
			get_template_directory_uri() . '/js/tiny-slider-module.js',
			array('jquery', 'wpeditor-main'),
			filemtime(__DIR__ . "/js/tiny-slider-module.js"),
			true
		);
		# }
		if (is_attachment() || (is_single() && has_post_format('gallery'))) {
			wp_enqueue_script('wpberita-tinyslider-gallery', get_template_directory_uri() . '/js/tiny-slider-gallery.js', array('wpeditor-main'), WPBERITA_VERSION, true);
		}
...
require get_template_directory() . '/inc/module-2.php';
...
# require get_template_directory() . '/inc/woocommerce.php';
require get_template_directory() . '/inc/topmenu.php';
require_once get_template_directory() . '/subtitle.php';
...
if (is_admin()) {
	...
	require get_template_directory() . '/inc/post-topic-taxonomy.php';
}

require_once get_template_directory() . '/hidelogin.php';
require_once get_template_directory() . '/wpadmin.php';
require_once get_template_directory() . '/splitpost/splitpost.php';
require_once get_template_directory() . '/inc/post-view.php';
# require_once __DIR__ . "/inc/iklan-listtable.php";
