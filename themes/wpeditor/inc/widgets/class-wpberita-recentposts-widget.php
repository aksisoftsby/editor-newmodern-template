...
class WpBerita_RecentPosts_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		// Box Widget.
		$wpb_boxwidget = ( ! empty( $instance['wpb_boxwidget'] ) ) ? wp_strip_all_tags( $instance['wpb_boxwidget'] ) : wp_strip_all_tags( 'disable' );

		echo $args['before_widget']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		if ( 'enable' === $wpb_boxwidget ) {
		...
		if ( $rp->have_posts() ) {
			if ( 'style_3' === $wpb_style || 'style_8' === $wpb_style ) {
				?>
				<div class="wpberita-list-gallery">
					...
					while ( $rp->have_posts() ) :
						$rp->the_post();
						$classes    = '';
						$thumb_size = 'medium-new';
						if ( 'style_8' === $wpb_style ) {
							if( 0 == $rp->current_post ) {
								$classes    = ' first-column';
								$thumb_size = 'large';
							}
						}
						?>
						<div class="list-gallery<?php echo esc_html( $classes ); ?>">
							<?php
				...
				<?php
			} elseif ( 'style_5' === $wpb_style || 'style_6' === $wpb_style || 'style_7' === $wpb_style ) {
				?>
					...
					while ( $rp->have_posts() ) :
						...
						if ( $count <= 1 ) {
							...
							<li>
								...
								if ( has_post_thumbnail() ) {
									?>
									<a class="post-thumbnail big-thumb" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>" aria-hidden="true" tabindex="-1">
										<?php
											the_post_thumbnail(
												'large',
												array(
								...
								if ( 'style_6' !== $wpb_style ) {
									...
									?>
								<div class="recent-content clearfix <?php echo esc_html( $classrc ); ?>">
									<?php
								}
							...
							<li>
								<?php
								$class = '';
								if ( 'style_6' !== $wpb_style && 'style_7' !== $wpb_style ) {
									if ( has_post_thumbnail() ) {
										?>
	...
	public function update( $new_instance, $old_instance ) {
		...
		// box widget.
		$instance['wpb_boxwidget'] = wp_strip_all_tags( $new_instance['wpb_boxwidget'] );
		// tag id.
	...
	public function form( $instance ) {
		...
		// box widget.
		$wpb_boxwidget = wp_strip_all_tags( $instance['wpb_boxwidget'] );
		// tag id.
		...
		<p>
			...
			<select class="widefat" id="<?php echo esc_html( $this->get_field_id( 'wpb_style', 'wpberita' ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'wpb_style' ) ); ?>">
				...
				<option value="style_5" <?php echo selected( $instance['wpb_style'], 'style_5', false ); ?>><?php esc_html_e( 'Default With 1 Big Thumbnail', 'wpberita' ); ?></option>
				<option value="style_6" <?php echo selected( $instance['wpb_style'], 'style_6', false ); ?>><?php esc_html_e( 'Just Title With 1 Big Thumbnail', 'wpberita' ); ?></option>
				<option value="style_7" <?php echo selected( $instance['wpb_style'], 'style_7', false ); ?>><?php esc_html_e( 'Number With Title With 1 Big Thumbnail', 'wpberita' ); ?></option>
				<option value="style_8" <?php echo selected( $instance['wpb_style'], 'style_8', false ); ?>><?php esc_html_e( 'gallery With 1 Big Thumbnail', 'wpberita' ); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'wpb_boxwidget' ) ); ?>"><?php esc_html_e( 'Box Widget', 'wpberita' ); ?></label>
			<select class="widefat" id="<?php echo esc_html( $this->get_field_id( 'wpb_boxwidget', 'wpberita' ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'wpb_boxwidget' ) ); ?>">
				<option value="disable" <?php echo selected( $instance['wpb_boxwidget'], 'disable', false ); ?>><?php esc_html_e( 'Disable Box', 'wpberita' ); ?></option>
				<option value="enable" <?php echo selected( $instance['wpb_boxwidget'], 'enable', false ); ?>><?php esc_html_e( 'Enable Box', 'wpberita' ); ?></option>
			</select>
		</p>
		<p>
