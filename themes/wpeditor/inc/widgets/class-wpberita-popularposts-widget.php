...
class WpBerita_PopularPosts_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $rp->have_posts() ) {
			?>
			<ul class="<?php echo ( 'style_2' === $wpb_style || 'style_3' === $wpb_style ) ? 'wpberita-list-widget' : 'wpberita-rp-widget'; ?>">
				<?php
				$count = 0;
				while ( $rp->have_posts() ) :
					$rp->the_post();
					...
					<li class="clearfix">
						<?php
						$class = '';
						if ( 'style_2' !== $wpb_style && 'style_3' !== $wpb_style ) {
						...
						if ( 'style_2' !== $wpb_style ) {
							...
							?>
						<div class="recent-content clearfix <?php echo esc_html( $class ); ?>">
							<?php
						}
							$number = '0';
							...
							wpberita_posted_on();
							echo '<span class="count-text">' . absint( $number ) . ' ' . esc_attr( $text ) . '</span>';
							echo '</div>';
